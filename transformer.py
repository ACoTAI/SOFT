...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, n_heads):
        ...
        self.d_k = self.d_v = d_model // n_heads if d_model//n_heads != 0 else d_model
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, seq_len, d_model=4, n_layers=2, n_heads=1, p_drop=0.1, d_ff=512):
        super(TransformerEncoder, self).__init__()
        self.d_model = d_model
        self.sinusoid_table = self.get_sinusoid_table(seq_len, d_model)  # (seq_len+1, d_model)
        # self.layer_normer = nn.LayerNorm(d_model, eps=1e-6)
        # layers
    ...
    def forward(self, inputs):
        ...
        for i in range(inputs.shape[0]):
            positions.append(self.sinusoid_table)
        outputs = inputs + torch.FloatTensor(positions).to(inputs.device)
        ...
        for layer in self.layers:
            # outputs = nn.LayerNorm(self.d_model, eps=1e-6)(outputs)
