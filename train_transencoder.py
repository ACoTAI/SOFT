...
def run_epoch(model, loader, cf, epoch, lr, optimizer=None, is_training=True, USE_CUDA=True, clip_norm=0):
    ...
    for i, data in enumerate(loader, 0):
        ...
        if epoch == 0:
            ...
            for j in range(step):
                virtual_inputs, vt_1 = loader.dataset.get_virtual_data(
                    virtual_queue, real_queue_idx, times[:, j], times[:, j + 1], times[:, 0], batch_size,
                    number_virtual,
                    real_postion[:, j])
                ...
                if is_training:
                    ...
                    with torch.no_grad():
                        ...
                        #     out = model.net(inputs, flo_out, ois_step)
                if j == 0:
                    flow_out_list = torch.unsqueeze(flo_out, 1) #flo_out.unsqueeze(1)
                    inputs_list = torch.unsqueeze(inputs, 1)
                else:
                    flow_out_list = torch.cat((flow_out_list, torch.unsqueeze(flo_out)), dim=1)
                    inputs_list = torch.cat((inputs_list, torch.unsqueeze(inputs)), dim=1)

            # inputs_flo_gyro = torch.cat((inputs_list, flow_out_list), dim=-1)

            out = model.net(inputs_list, flow_out_list, ois_step)
...
def train(args=None):
    ...
    for count in range(epoch_load + 1, epochs + 1):
        ...
        train_loss = run_epoch(model, train_loader, cf, count, lr, optimizer=optimizer, clip_norm=clip_norm,
