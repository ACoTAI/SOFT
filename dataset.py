...
def get_data_loader(cf, size, num_workers, no_flo = False):
...
def get_inference_dataset(cf, data_path,simCLR=False, no_flo = False):
    resize_ratio = cf["data"]["resize_ratio"]
...
class Dataset_Gyro(Dataset):
    def __init__(self, path, sample_freq = 33*1000000, number_real = 10, time_train = 2000*1000000, \
        ...
        self.train_simCLR_transform = transforms.Compose([
    ...
    def process_one_video(self, path):
        ...
        for f in files:
            ...
            if "frame" in f and "txt" in f:
                ...
                dvs_data.gyro = LoadGyroData(file_path)  # this is relative data not absolute
    ...
    def __getitem__(self, idx):
        ...
        if self.no_flo:
            ...
            #     while True:
            #         minus_id = random.randint(1, 16)
            #         if idx == minus_id:
            #             continue
            #         else:
            #             break
            #     minus_first_id = random.randint(0, self.data[minus_id].length - self.number_train) + 1
            #     minus_flo, minus_flo_back = self.load_flo(minus_id, minus_first_id)
            #     flo = np.concatenate((flo, minus_flo), axis=0)
            #     flo_back = np.concatenate((flo_back, minus_flo_back), axis=0)
            # else:
            flo, flo_back = self.load_flo(idx, first_id)
            # if self.simCLR:
            #     flow = np.concatenate((flo, flo_back), axis=3)
            #     flo = self.train_simCLR_transform(flow)
            #     flo_back = self.train_simCLR_transform(flow)

            # flo, flo_back = self.load_flo(idx, first_id)

        return inputs, times, flo, flo_back, real_projections, real_postion, ois, idx, self.number_train
...
def get_timestamp(frame_data, idx):
    ...
    timestmap_ns = metadata["timestamp_ns"] + metadata["rs_time_ns"] * 0.5  # 0 + 3 *0.5
    return timestmap_ns
