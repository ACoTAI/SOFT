...
def get_optimizer(optimizer, model, init_lr, cf):
    # if optimizer == "adam":
    if optimizer == "adam":
        optimizer = torch.optim.Adam(chain(model.net.parameters(), filter(lambda p: p.requires_grad, model.unet.parameters())), lr=init_lr, weight_decay=cf["train"]["weight_decay"])
    elif optimizer == "sgd":
        optimizer = torch.optim.SGD(chain(model.net.parameters(), filter(lambda p: p.requires_grad, model.unet.parameters())), lr=init_lr, momentum=cf["train"]["momentum"])
    return optimizer
